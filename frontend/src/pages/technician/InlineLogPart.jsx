...
export default function InlineLogPart({ job, onSuccess }) {
  ...
  const handleLogPart = async (e) => {
    e.preventDefault();
    if (!partItemId) {
      setPartError("Please scan or select a part first.");
      return;
    }
    setPartError("");
    setSavingPart(true);
    try {
      ...
      // reset form
      setPartItemId(""); setPartBatch(null); setPartSerial(null); setPartQty(1); setPartInfo("");
