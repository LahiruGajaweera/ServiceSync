...
export default function ChatbotWidget() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      {isOpen ? (
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-80 sm:w-96 flex flex-col h-[500px] border border-gray-100 dark:border-gray-800 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
          {/* Header */}
            ...
            {messages.map((msg, index) => (
              <div key={index} className={`flex ${msg.isBot ? 'justify-start' : 'justify-end'}`}>
                <div className={`flex max-w-[85%] gap-2 ${msg.isBot ? 'flex-row' : 'flex-row-reverse'}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${msg.isBot ? 'bg-blue-100 text-blue-600' : 'bg-gray-200 text-gray-600 dark:text-gray-300'}`}>
                    {msg.isBot ? <Bot size={16} /> : <User size={16} />}
                  </div>
                  <div className={`p-3 rounded-2xl text-sm ${msg.isBot ? 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-800 dark:text-gray-100 rounded-tl-none shadow-sm' : 'bg-blue-600 text-white rounded-tr-none shadow-md'}`}>
                    ...
                    {msg.jobData && (
                      <div className="mt-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-3 w-full shadow-sm">
                        <div className="flex justify-between items-start mb-2 gap-2">
                           <span className="font-bold text-gray-800 dark:text-gray-100 text-[13px]">{msg.jobData.job_id}</span>
                        ...
                        <div className="text-[12px] text-gray-600 dark:text-gray-300 space-y-1">
                          <p><span className="font-semibold text-gray-700 dark:text-gray-200">Device:</span> {msg.jobData.device_brand} {msg.jobData.device_model}</p>
                          <p><span className="font-semibold text-gray-700 dark:text-gray-200">Issue:</span> {msg.jobData.fault_category?.replace(/_/g, " ")}</p>
                          {msg.jobData.estimated_cost != null && (
                            <p><span className="font-semibold text-gray-700 dark:text-gray-200">Est. Cost:</span> LKR {Number(msg.jobData.estimated_cost).toLocaleString()}</p>
                          )}
                        </div>
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="flex max-w-[85%] gap-2 flex-row">
                  ...
                  <div className="p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 rounded-2xl rounded-tl-none shadow-sm text-sm flex gap-1 items-center">
          ...
          <div className="p-3 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-800">
            <form onSubmit={handleSend} className="flex gap-2 items-center">
              <input
                ...
                className="flex-1 bg-gray-100 border-transparent focus:bg-white dark:bg-gray-800 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 rounded-full px-4 py-2.5 text-sm outline-none transition-all"
                disabled={isLoading}
              />
              <button
                type="submit"
